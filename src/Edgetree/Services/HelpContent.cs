...
public static class HelpContent
{
    ...
    private static Row R(
        string koGesture, string koMeaning,
        string enGesture, string enMeaning,
        string zhGesture, string zhMeaning)
        => Strings.IsSimplifiedChinese
            ? new Row(zhGesture, zhMeaning)
            : Strings.IsEnglish
                ? new Row(enGesture, enMeaning)
                : new Row(koGesture, koMeaning);

    private static string T(string ko, string en, string zh)
        => Strings.IsSimplifiedChinese ? zh : Strings.IsEnglish ? en : ko;
    ...
    public static string TipsTitle
        => T("처음 사용자 TIP", "New here? Start with these", "新用户？从这些开始");
    ...
    public static IReadOnlyList<string> Tips() => new[]
    {
        ...
        T("자주 쓰는 파일 형식(확장자) 지정하기 (제외 확장자 입력시 최우선 적용)",
          "Set the file kinds you work with - anything you type as excluded wins over the rest", "设置你常用的文件类型；输入到排除列表的扩展名优先级最高"),
        T("자주 가는 곳은 북마크로", "Bookmark the places you keep going back to", "把经常访问的位置添加为书签"),
        ...
        T("재생 중인 음악·보던 영상은 트리 하단 줄에서 바로 복귀 (음악은 패널을 닫아도 계속 재생)",
          "The row at the foot of the tree takes you back to the music still playing, or the film you were watching - and music carries on with the panel closed", "在树底部一行中返回仍在播放的音乐或正在观看的视频；关闭面板后音乐仍会继续播放"),
        T("썸네일 캐싱 활성화로 이미지 많은 폴더 빠르게 관리",
          "Turn thumbnail caching on and move through big image folders fast", "开启缩略图缓存，更快浏览包含大量图片的文件夹"),
        T("원하는 색상 선택", "Pick the colors you want", "选择所需的颜色"),
        T("설정한 상태를 프리셋으로 저장 (제목 표시줄 우클릭 메뉴)",
          "Save a setup as a preset (right-click the title bar)", "将当前设置保存为预设（右键标题栏）"),
    };
    ...
    public static IReadOnlyList<Section> Build() => new[]
    {
            ...
            new[]
            {
                new Group(string.Empty, new[]
                {
                    R("가장자리 혹은 손잡이에 커서 이동", "다시 펼치기",
                      "Point at the screen edge, or at the handle", "Bring it back", "指向屏幕边缘或把手", "重新展开"),
                    ...
                    R("트레이 아이콘 클릭 · 프로그램 다시 실행", "숨은 앱 다시 펼치기",
                      "Click the tray icon, or run the app again", "Opens a hidden app back up", "点击托盘图标或再次运行程序", "重新展开隐藏的应用"),
                    R("핀 클릭", "고정하기 ↔ 자동 숨김",
                      "Click the pin", "Stay open ↔ auto-hide", "点击图钉", "固定 ↔ 自动隐藏"),
                    ...
                    R("제목 표시줄 최소화 아이콘", "트레이 옵션과 모드에 따라 다름",
                      "The minimize icon in the title bar", "Depends on the tray option and the mode", "标题栏中的最小化图标", "取决于托盘选项和当前模式"),
                    ...
                    R("  - 트레이 옵션 켜짐", "트레이로 최소화",
                      "  - Tray option on", "Minimize to the tray", "  - 托盘选项开启", "最小化到托盘"),
                    ...
                    R("  - 트레이 꺼짐 · 도킹 상태", "숨김",
                      "  - Tray off, docked", "Hide it", "  - 托盘关闭 · 停靠状态", "隐藏"),
                    R("  - 트레이 꺼짐 · 창 모드", "작업 표시줄로 최소화",
                      "  - Tray off, window mode", "Minimize to the taskbar", "  - 托盘关闭 · 窗口模式", "最小化到任务栏"),
                    ...
                    R("위 · 아래 가장자리 드래그", "높이와 위치 (짧은 밴드)",
                      "Drag the top or bottom edge", "Height, and where the band sits", "拖动上边缘或下边缘", "调整高度和位置（短条带）"),
                    R("그 가장자리 더블클릭", "화면 높이 전체로 복원",
                      "Double-click that edge", "Back to the full height", "双击该边缘", "恢复为全屏高度"),
                    R("드래그 중 Shift · Ctrl", "격자에 맞춰 스냅 (Ctrl이 더 작은 격자)",
                      "Shift · Ctrl while dragging", "Snap to a grid - Ctrl uses a smaller one", "拖动时按住 Shift · Ctrl", "吸附到网格；Ctrl 使用更小的网格"),
                }),
            }),

        new Section(
            T("창 모드", "Window mode", "窗口模式"),
            new[]
            {
                new Group(string.Empty, new[]
                {
                    R("제목 표시줄 드래그", "창으로 분리 · 이동",
                      "Drag the title bar", "Pull it off, and move it", "拖动标题栏", "分离为窗口 · 移动"),
                    R("제목 표시줄 더블클릭", "최대화 · 복원",
                      "Double-click the title bar", "Maximize · restore", "双击标题栏", "最大化 · 还原"),
                    R("가장자리 · 모서리 드래그", "크기 조절",
                      "Drag an edge or corner", "Resize", "拖动边缘或角落", "调整大小"),
                    R("가장자리로 드래그", "다시 도킹",
                      "Drag to a screen edge", "Dock it again", "拖到屏幕边缘", "重新停靠"),
                    R("핀 클릭", "도킹 상태로 복귀",
                      "Click the pin", "Go back to being docked", "点击图钉", "恢复停靠状态"),
                }),
            }),
            ...
            new[]
            {
                new Group(string.Empty, new[]
                {
                    R("제목 표시줄 우클릭", "저장된 프리셋 목록 · 프리셋 추가 (최대 5개)",
                      "Right-click the title bar", "The setups you saved, and Add preset - up to five", "右键标题栏", "已保存的预设列表 · 添加预设（最多 5 个）"),
                    ...
                    R("저장 항목", "현재 앱 설정 전체를 그대로 저장",
                      "What it holds", "Everything you have set up, exactly as it stands", "保存内容", "完整保存当前应用设置，不改变当前状态"),
                    R("이름 옆 ›", "적용 · 덮어쓰기 · 이름 바꾸기 · 삭제",
                      "The › beside a name", "Apply · overwrite · rename · delete", "名称旁的 ›", "应用 · 覆盖 · 重命名 · 删除"),
                    ...
                    R("Ctrl+1 ~ 5", "해당 번호의 프리셋으로 전환",
                      "Ctrl+1 ~ 5",
                      "Go to that preset", "Ctrl+1 ~ 5", "切换到对应编号的预设"),
                    R("Ctrl+Shift+S", "현재 선택된 프리셋에 확인 없이 바로 덮어씌움",
                      "Ctrl+Shift+S",
                      "Overwrites the preset you are in, straight away, without asking", "Ctrl+Shift+S", "无需确认，直接覆盖当前选中的预设"),
                }),
            }),
        ...
        new Section(
            ...
            new[]
            {
                new Group(string.Empty, new[]
                {
                    R("↑ ↓", "위 · 아래 행 이동", "↑ ↓", "Previous · next row", "↑ ↓", "移动到上一行 · 下一行"),
                    ...
                    R("PageUp · PageDown", "화면에 나와 있는 폴더로만 위 · 아래 이동",
                      "PageUp · PageDown",
                      "Up · down through the folders on screen, skipping the files - nothing gets expanded", "PageUp · PageDown", "只在屏幕上显示的文件夹之间上下移动，跳过文件；不会展开任何内容"),
                    R("← →", "접기 · 펼치기", "← →", "Collapse · expand", "← →", "折叠 · 展开"),
                    // 2026-08-17: 열린 폴더를 눌러도 안 접히게 바뀌었으므로 이
                    // 두 줄이 필요해졌다. 접는 방법이 둘(다시 누르기 · 펼침기호)이고
                    // 어느 쪽도 화면에 안 적혀 있다.
                    R("폴더 클릭", "접힌 폴더는 펼치기 · 열린 폴더는 선택만, 다시 클릭하면 접기",
                      "Clicking a folder",
                      "A closed folder opens; an open one is only selected - press it again to close", "点击文件夹", "折叠的文件夹会展开；已展开的文件夹只会被选中，再次点击可折叠"),
                    ...
                    R("클릭 시 바로 펼침을 끄면", "첫 클릭은 선택 · 두 번째 클릭이 펼치기 · 접기",
                      ...
                      "The first click selects; the second one opens or closes", "关闭单击即展开后", "第一次点击选中；第二次点击展开或折叠"),
                    R("펼침기호 클릭", "설정과 무관하게 누를 때마다 바로 접기 · 펼치기",
                      "Clicking the expand arrow",
                      "Opens and closes on every press, whatever that setting says", "点击展开箭头", "无论该设置如何，每次点击都会立即展开或折叠"),
                    ...
                    R("제목 표시줄의 전체 접기 아이콘", "펼친 폴더를 전체 접음, 다시 클릭하면 펼침 상태 복원",
                      ...
                      "Folds every open folder; press it again to put them back", "标题栏中的全部折叠图标", "折叠所有已展开的文件夹；再次点击可恢复展开状态"),
                    R("전체 접기 아이콘 Shift+클릭", "펼침 상태를 완전히 접음 (복원 안 됨)",
                      "Shift-clicking the Collapse All icon",
                      "Folds for good (no restore)", "Shift+点击全部折叠图标", "彻底折叠展开状态（无法恢复）"),
                    ...
                    R("Shift+Backspace", "위와 같음 (복원 안 됨)",
                      "Shift+Backspace", "The same, from the keyboard (no restore)", "Shift+Backspace", "同上，使用键盘操作（无法恢复）"),
                    ...
                    R("Ctrl+← · Ctrl+→", "사용한 폴더로 뒤로 · 앞으로 (경로 표시줄의 < > 와 같음)",
                      "Ctrl+← · Ctrl+→",
                      "Back · forward through the folders you have used - the same as the path bar's < >", "Ctrl+← · Ctrl+→", "在已访问的文件夹之间后退 · 前进（与路径栏的 < > 相同）"),
                    ...
                    R("마우스 앞 · 뒤 버튼", "트리 위에서 클릭하면 사용한 폴더로 뒤로 · 앞으로",
                      ...
                      "Over the tree, back · forward through the folders you have used", "鼠标前进 · 后退按钮", "在树上点击可在已访问的文件夹之间后退 · 前进"),
                    R("경로 표시줄의 목록 버튼", "사용한 폴더들을 확인하고 바로 이동",
                      "The list button on the path bar",
                      "See the folders you have used, and go straight to one", "路径栏的列表按钮", "查看已访问的文件夹并直接跳转"),
                    R("Ctrl + 휠", "빠른 스크롤", "Ctrl + wheel", "Scroll about five times faster", "Ctrl + 滚轮", "快速滚动"),
                    R("Ctrl + / −", "글자 크기 조절", "Ctrl + / −", "Text size", "Ctrl + / −", "调整文字大小"),
                    R("Enter", "열기", "Enter", "Open", "Enter", "打开"),
                    R("F2", "이름 바꾸기", "F2", "Rename", "F2", "重命名"),
                    R("F5", "새로고침", "F5", "Refresh", "F5", "刷新"),
                    R("F7", "새 폴더", "F7", "New folder", "F7", "新建文件夹"),
                    R("Del", "휴지통으로 삭제", "Del", "Delete to the Recycle Bin", "Del", "移到回收站"),
                    R("Shift+Del", "완전 삭제", "Shift+Del", "Delete permanently", "Shift+Del", "永久删除"),
                    R("Ctrl+C · Ctrl+X · Ctrl+V", "복사 · 잘라내기 · 붙여넣기",
                      "Ctrl+C · Ctrl+X · Ctrl+V", "Copy · cut · paste", "Ctrl+C · Ctrl+X · Ctrl+V", "复制 · 剪切 · 粘贴"),
                    R("Ctrl+Shift+C", "경로 복사", "Ctrl+Shift+C", "Copy path", "Ctrl+Shift+C", "复制路径"),
                    R("Alt+Enter", "속성", "Alt+Enter", "Properties", "Alt+Enter", "属性"),
                    ...
                    R("드래그", "같은 드라이브는 이동, 다른 드라이브는 복사",
                      "Drag", "Move within a drive, copy between drives", "拖动", "同一驱动器内移动，不同驱动器之间复制"),
                    R("다른 탐색기로 드래그", "복사", "Drag to another explorer", "Copy", "拖到其他资源管理器", "复制"),
                    ...
                    R("드래그 중 Shift · Ctrl", "이동 · 복사 고정",
                      "Shift · Ctrl while dragging", "Force a move · force a copy", "拖动时按住 Shift · Ctrl", "强制移动 · 强制复制"),
                    R("Ctrl+Alt+K", "북마크 표시 · 해제", "Ctrl+Alt+K", "Bookmark, or take it back", "Ctrl+Alt+K", "添加书签 · 取消书签"),
                    R("Ctrl+Alt+L · J", "다음 · 이전 북마크로 이동", "Ctrl+Alt+L · J", "Next · previous bookmark", "Ctrl+Alt+L · J", "跳转到下一个 · 上一个书签"),
                    ...
                    R("북마크 패널에서 행 드래그", "순서 변경 · 번호가 곧 Ctrl+Alt+L 순서",
                      ...
                      "Reorders it - the numbers are the Ctrl+Alt+L order", "拖动书签面板中的行", "重新排序；编号就是 Ctrl+Alt+L 的顺序"),
                    ...
                    R("우클릭 → 숨기기", "트리에서 폴더나 드라이브를 감춤, 검색에서는 보이나 작업 시 숨김 해제 필요",
                      "Right-click → Exclude",
                      "Takes a folder or a drive out of the tree; search still finds it, but working on it means putting it back first", "右键 → 排除", "从树中隐藏文件夹或驱动器；搜索仍能找到它，但操作前必须先恢复显示"),
                    ...
                    R("빈 곳 우클릭",
                      ...
                      "Refresh everything · New folder · Bookmark · Excluded folders · Network Locations · File types", "右键空白处", "全部刷新 · 新建文件夹 · 添加书签 · 已排除文件夹 · 网络位置 · 文件类型"),
                }),
            }),

        new Section(
            T("필터와 정렬", "Filtering and sorting", "筛选和排序"),
            new[]
            {
                new Group(string.Empty, new[]
                {
                    R("아래쪽 필터 버튼 줄", "종류별 필터링",
                      "The filter buttons at the bottom", "Show only some kinds of file", "底部的筛选按钮行", "按类型筛选"),
                    ...
                    R("필터 변경 시", "트리를 접고 맨 위로 이동",
                      "When a filter changes", "The tree folds and goes to the top", "更改筛选条件时", "折叠树并回到顶部"),
                    R("필터 버튼 → 사용자 지정", "확장자 직접 입력 ( *. 없이 쉼표로 구분해서 여러 개 입력)",
                      "A filter button → Custom",
                      "Type the extensions yourself - no *. , and comma-separated for several", "筛选按钮 → 自定义", "直接输入扩展名（不要输入 *.，多个扩展名用逗号分隔）"),
                    // 빈 폴더로 오해하고 지우는 것을 막는 줄이므로 필터 쪽에 둔다 -
                    // 이 줄을 보는 사람은 필터를 켜 둔 사람이다.
                    R("필터가 전부 감춘 폴더", "감춰진 파일 개수를 폴더 안에 표시",
                      "A folder the filter empties",
                      "Shows how many files it is hiding, in the folder's own place", "被筛选条件隐藏全部内容的文件夹", "在文件夹原本的位置显示被隐藏的文件数量"),
                    ...
                    R("트리 빈 곳 우클릭 → 표시할 파일 형식", "필터 버튼 줄과 같은 목록",
                      ...
                      "The same list the filter buttons carry", "右键树的空白处 → 文件类型", "与筛选按钮相同的列表"),
                    R("폴더 행의 정렬 아이콘", "그 폴더의 개별 정렬, 하위 폴더는 상위 폴더의 정렬을 따름(단, 하위 폴더를 개별 정렬할 경우 별개로 유지)",
                      "The sort icon on a folder row",
                      "Sort that folder its own way - subfolders follow, unless they have their own", "文件夹行上的排序图标", "单独设置该文件夹的排序方式；子文件夹跟随，除非它们有自己的排序方式"),
                    ...
                    R("정렬 메뉴 → 부모 폴더 따르기",
                      ...
                      "On by default; picking a field of its own clears it, and choosing it again drops that folder's own setting", "排序菜单 → 跟随父文件夹", "默认开启；选择单独排序字段会关闭它，再次选择会删除该文件夹自己的设置"),
                    R("옵션 → 정렬 기준", "전체 기본 정렬", "Options → Sort by", "The default for everything", "选项 → 排序方式", "全部内容的默认排序方式"),
                    ...
                    R("더 보기 · 접기", "나머지 항목 펼치기 · 되접기 (그 부모 폴더가 선택됨)",
                      "Show more · Show fewer", "Reveal the rest, or fold it back - its parent folder gets selected", "显示更多 · 显示更少", "展开剩余项目，或重新折叠；其父文件夹会被选中"),
                }),
            }),
        ...
        new Section(
            ...
            new[]
            {
                new Group(string.Empty, new[]
                {
                    R("Ctrl+F", "검색 열기", "Ctrl+F", "Open search", "Ctrl+F", "打开搜索"),
                    R("제목 표시줄 돋보기", "검색 열기 · 닫기",
                      "The magnifier in the title bar", "Open · close search", "标题栏中的放大镜", "打开 · 关闭搜索"),
                    R("폴더 아이콘", "검색 범위 선택", "The folder icon", "Choose where to look", "文件夹图标", "选择搜索范围"),
                    ...
                    R("검색창의 정렬 아이콘", "폴더별 묶기 · 이름 · 수정한 날짜 · 오름차순 · 내림차순, 묶기와 정렬은 각각 적용",
                      ...
                      "Group by folder · Name · Date modified · Ascending · Descending - the grouping and the sort apply separately", "搜索栏中的排序图标", "按文件夹分组 · 名称 · 修改日期 · 升序 · 降序；分组和排序分别生效"),
                    ...
                    R("결과 클릭", "트리의 해당 항목으로 이동",
                      "Click a result", "Go to it in the tree", "点击结果", "跳转到树中的对应项目"),
                    R("결과 클릭 · ↑ ↓ (멀티미디어 패널 열림)", "검색 결과 간 이동",
                      "Click a result · ↑ ↓", "(with the multimedia panel open) Move through the results", "点击结果 · ↑ ↓（多媒体面板已打开）", "在搜索结果之间移动"),
                    R("결과 더블클릭 (멀티미디어 패널 열림)", "트리의 해당 항목으로 이동",
                      "Double-click a result", "(with the multimedia panel open) Go to it in the tree", "双击结果（多媒体面板已打开）", "跳转到树中的对应项目"),
                    R("결과 표시 수", "기본 1,000개 · 목록 끝 더 보기로 1,000개씩 추가",
                      "How many results show", "1,000 at a time - \"Show more\" at the end adds another 1,000", "显示结果数量", "默认每次显示 1,000 个；列表末尾的“显示更多”每次再增加 1,000 个"),
                    ...
                    R("상태줄 오른쪽 ↻", "다시 인덱싱, 변경된 내용이 있을 경우 파란 점 표시",
                      "The ↻ on the status line", "Reindex - a blue dot means the folder has changed", "状态栏右侧的 ↻", "重新建立索引；蓝点表示文件夹已发生变化"),
                    R("Esc · Ctrl+E", "검색 닫기", "Esc · Ctrl+E", "Close search", "Esc · Ctrl+E", "关闭搜索"),
                }),
            }),
        ...
        new Section(
            ...
            T("멀티미디어 패널", "Multimedia Panel", "多媒体面板"),
            new[]
            {
                ...
                new Group(string.Empty, new[]
                {
                    ...
                    R("옵션 → 멀티미디어 패널 → 더블클릭으로 열기",
                      ...
                      "A double-click in the tree opens the file here instead of in your usual program", "选项 → 多媒体面板 → 双击打开", "在树中双击文件时，在面板中打开，而不是使用默认程序"),
                    ...
                    R("전체 화면에서 우클릭", "파일 항목 대신 그림 크기 · 내비게이터 · 썸네일 바와 제목 표시줄 메뉴 (프리셋 · 도움말 · 다시 시작 · 종료)",
                      ...
                      "Picture size, navigator and thumbnail bar instead of the file items, plus the title bar's menu - presets, help, restart, quit", "全屏时右键", "显示图片大小、导航器和缩略图栏，而不是文件项目；还包括标题栏菜单：预设、帮助、重启、退出"),
                    ...
                    R("전체 화면에서 우클릭 → 바탕화면 채우기",
                      ...
                      "Off keeps the window's own size and fills that instead; on (the default) grows it to the whole screen", "全屏时右键 → 填充桌面", "关闭后保持窗口自身大小，只让图片填满窗口；开启时（默认）扩展到整个屏幕"),
                    ...
                    R("전체 화면에서 위쪽 띠 드래그",
                      ...
                      "Moves the window; docked, it undocks and then follows", "全屏时拖动顶部条带", "移动窗口；停靠状态下会先取消停靠再移动"),
                }),

                new Group(T("이미지", "Images", "图片"), new[]
                {
                    R("휠", "확대 · 축소", "Wheel", "Zoom in · out", "滚轮", "放大 · 缩小"),
                    R("Ctrl+휠 · Shift+휠", "정밀 확대 축소 - Shift 조합시 더 세밀하게",
                      "Ctrl+wheel · Shift+wheel", "Precision zoom - finer with Shift", "Ctrl+滚轮 · Shift+滚轮", "精确缩放；配合 Shift 时步进更细"),
                    ...
                    R("표시 형식", "Windows가 미리보기를 만들 수 있는 그림 (PSD · RAW · JXL 등 포함)",
                      "Formats it shows",
                      "Any picture Windows can make a thumbnail of - PSD, RAW, JXL and the rest", "显示格式", "Windows 能生成缩略图的图片（包括 PSD · RAW · JXL 等）"),
                    ...
                    R("SVG", "Windows에 렌더링 기능 설치돼 있을 때만 표시",
                      "SVG", "Shows only where Windows has something installed that can draw it", "SVG", "仅当 Windows 已安装可渲染 SVG 的组件时显示"),
                    R("드래그", "확대 상태에서 화면 이동", "Drag", "Move around, when zoomed in", "拖动", "放大后移动画面"),
                    ...
                    R("내비게이터", "확대 시 우측 하단에 표시",
                      "The navigator", "Bottom right, once zoomed in", "导航器", "放大后显示在右下角"),
                    R("좌우로 길게 드래그", "이전 · 다음 이미지", "Drag sideways, a good push", "Previous · next image", "向左或向右大幅拖动", "上一张 · 下一张图片"),
                    ...
                    R("마우스 앞 · 뒤 버튼", "이전 · 다음 이미지",
                      "Mouse back · forward buttons", "Previous · next image", "鼠标前进 · 后退按钮", "上一张 · 下一张图片"),
                    R("더블클릭", "맞춤 ↔ 1:1 전환", "Double-click", "Fit ↔ actual size", "双击", "适应 ↔ 实际大小"),
                    ...
                    R("채우기", "패널을 꽉 채우고 넘치는 부분은 잘라냄",
                      "Fill", "Fills the panel and crops the overflow", "填充", "填满面板，超出部分裁剪"),
                    R("Wheel Click · Enter", "전체 화면 (우클릭 메뉴에서 방식 선택)",
                      "Wheel Click · Enter", "Full screen (pick which kind in the right-click menu)", "单击滚轮 · Enter", "全屏（在右键菜单中选择模式）"),
                    R("Esc", "전체 화면 종료", "Esc", "Leave full screen", "Esc", "退出全屏"),
                    ...
                    R("Backspace", "패널 접기 (패널이 펼쳐져 있을 때)",
                      "Backspace", "Fold the panel away, while it is open", "Backspace", "折叠面板（面板展开时）"),
                    // 목록의 한 줄 이동은 일부러 안 적었다(사용자, 2026-08-22):
                    // 격자에서 ↑↓가 줄을 걷는 것은 너무 자연스러워서 사소하고,
                    // 재생 중 볼륨 등 다른 ↑↓ 설명과 부딪혀 오히려 헷갈린다.
                    R("↑ ↓", "이전 · 다음 항목", "↑ ↓", "Previous · next item", "↑ ↓", "上一项 · 下一项"),
                    R("← →", "썸네일 바 켜져 있을 때 이전 · 다음",
                      "← →", "Previous · next, while the thumbnail bar is open", "← →", "缩略图栏打开时切换上一项 · 下一项"),
                    ...
                    R("F8 · 우클릭 → 슬라이드 쇼", "폴더의 이미지를 차례로 표시 (2장 이상일 때)",
                      "F8 · Right-click → Slideshow", "Turn the folder's images over, one by one (needs two or more)", "F8 · 右键 → 幻灯片放映", "依次显示文件夹中的图片（至少需要 2 张）"),
                    R("슬라이드 쇼 중 클릭 · ↑ ↓ · 트리 선택", "쇼 종료, 보고 있던 사진에서 멈춤",
                      "Click, ↑ ↓, or pick a row while it runs", "Ends the show, staying on the picture you were looking at", "幻灯片放映中点击 · ↑ ↓ · 选择树项目", "结束放映并停留在当前查看的图片"),
                    R("F9 · ⋯ → 멀티미디어 패널 → 시계와 날짜", "패널 위에 시각·날짜·요일을 표시",
                      "F9 · ⋯ → Multimedia panel → Clock and date", "Puts the time, date and day over the panel", "F9 · ⋯ → 多媒体面板 → 时钟和日期", "在面板上显示时间、日期和星期"),
                    ...
                    R("⋯ → 멀티미디어 패널 → 시계 크기", "50% ~ 150% (패널 크기에 대한 비율)",
                      "⋯ → Multimedia panel → Clock size", "50% to 150%, as a share of the panel", "⋯ → 多媒体面板 → 时钟大小", "50% 至 150%（相对于面板大小的比例）"),
                    ...
                    R("우클릭 → 배경 설정", "앱이 위치한 모니터의 배경화면으로 지정",
                      "Right-click → Set as wallpaper", "Sets it on the monitor the app is on", "右键 → 设置为壁纸", "设置为应用所在显示器的壁纸"),
                }),

                new Group(T("영상", "Film", "视频"), new[]
                {
                    R("Space · 화면 클릭", "재생 · 정지", "Space · click the video", "Play · pause", "Space · 点击画面", "播放 · 暂停"),
                    R("← →", "5초 뒤 · 앞", "← →", "Back · forward 5 seconds", "← →", "后退 · 前进 5 秒"),
                    R("마우스 앞 · 뒤 버튼", "10초 뒤 · 앞", "Mouse back · forward buttons", "Back · forward 10 seconds", "鼠标前进 · 后退按钮", "后退 · 前进 10 秒"),
                    R("같은 영상 재생 시", "마지막 위치부터 이어보기",
                      "Play the same file again", "It carries on from where you left off", "再次播放同一视频", "从上次离开的位置继续播放"),
                    ...
                    R("보다가 다른 폴더로 이동", "트리 하단에 영상 이름 표시, 클릭하면 보던 위치에서 이어보기",
                      ...
                      "The film's name waits at the foot of the tree - press it to carry on where you left off", "观看时移动到其他文件夹", "视频名称会显示在树底部；点击后从上次位置继续播放"),
                    R("Home", "처음부터", "Home", "Back to the start", "Home", "回到开头"),
                    R("P · Insert", "영상 북마크 표시 · 해제",
                      "P · Insert", "Bookmark this moment, or take it back", "P · Insert", "添加视频书签 · 取消书签"),
                    R("우클릭 → 영상 북마크 목록", "표시 위치로 이동 · 전체 삭제",
                      "Right-click → Video bookmarks", "Jump to one, or clear them all", "右键 → 视频书签列表", "跳转到标记位置 · 全部删除"),
                    R("재생 중 ↑ ↓", "볼륨 조절 (일시정지하면 항목 이동으로 복귀)",
                      "↑ ↓ while playing", "Volume - they walk the folder again once it is paused", "播放时 ↑ ↓", "调节音量；暂停后恢复为切换项目"),
                    R("재생 중 M", "음소거", "M while playing", "Mute", "播放时 M", "静音"),
                    ...
                    R("우클릭 → 영상 크기 → 채우기", "패널을 꽉 채우고 넘치는 부분은 잘라냄",
                      ...
                      "Fills the panel and crops the overflow", "右键 → 视频大小 → 填充", "填满面板，超出部分裁剪"),
                    // 바로 위 줄의 반대편. 그 줄은 창 안에서 영상을 어떻게 놓을지
                    // 정하고, 이 줄은 영상에 창을 맞춘다.
                    R("F · 우클릭 → 창을 영상에 맞춤", "창 높이를 영상 비율에 맞춤, 위아래 검은 띠가 없어짐 (창 모드)",
                      "F · right-click → Fit window to video",
                      "Sets the window's height to the film's proportions - the black bands go (window mode)", "F · 右键 → 使窗口适应视频", "按视频比例调整窗口高度，去除上下黑边（窗口模式）"),
                    R("우클릭 → 자막", "활성화 · 크기 · 위치 · 싱크 조절",
                      "Right-click → Subtitles", "On, size, position, and sync", "右键 → 字幕", "启用 · 大小 · 位置 · 同步调整"),
                    ...
                    R("< > (자막 있을 때)", "자막 싱크를 0.5초씩 앞 · 뒤로, 누르고 있으면 계속",
                      ...
                      "Subtitle sync half a second at a time - hold either one to keep going", "< >（有字幕时）", "字幕同步每次前进 · 后退 0.5 秒；按住任一按钮可连续调整"),
                    ...
                    R("우클릭 → HDR 색 보정", "밝기 · 채도 · 대비 조절, 흰빛으로 뜨는 HDR 영상 보정",
                      ...
                      "Brightness, saturation and contrast - for the HDR films that arrive washed out", "右键 → HDR 色彩校正", "调整亮度 · 饱和度 · 对比度，修正发白的 HDR 视频"),
                    ...
                    R("전체 화면에서 아래쪽", "컨트롤 패널 표시",
                      "Full screen, point at the bottom", "Bring the playback controls back", "全屏时指向底部", "显示播放控制栏"),
                    ...
                    R("재생 형식",
                      ...
                      "Formats it plays",
                      "mp4 · m4v · mov · avi · wmv · mkv · webm · mpg · mpeg · ts · m2ts · mts · flv · 3gp · asf · wm · qt · 3g2 · m2t", "播放格式", "mp4 · m4v · mov · avi · wmv · mkv · webm · mpg · mpeg · ts · m2ts · mts · flv · 3gp · asf · wm · qt · 3g2 · m2t"),
                    R("코덱", "외부 코덱 사용 (필요 시 직접 설치)",
                      "Codecs", "Whatever is installed on the PC - add one if a file needs it", "编解码器", "使用电脑上已安装的编解码器；如果文件需要，可自行安装"),
                    ...
                    R("AV1", "현재 미지원 (코덱에 따라 다를 수 있음)",
                      "AV1", "Not supported at present - may differ with the codecs installed", "AV1", "目前不支持；实际情况可能取决于已安装的编解码器"),
                    ...
                    R("재생 오류 시", "기본 프로그램으로 열기 권장 (트리에서 더블클릭 · Enter)",
                      "When it will not play", "Better opened in your usual player (double-click or Enter, in the tree)", "播放失败时", "建议使用默认程序打开（在树中双击 · 按 Enter）"),
                }),
                ...
                new Group(T("음악", "Music", "音乐"), new[]
                {
                    R("재생 형식", "mp3 · wav · flac · m4a · m4b · aac · wma",
                      "Formats it plays", "mp3 · wav · flac · m4a · m4b · aac · wma", "播放格式", "mp3 · wav · flac · m4a · m4b · aac · wma"),
                    R("앨범아트 클릭", "재생 · 정지", "Click the album art", "Play · pause", "点击专辑封面", "播放 · 暂停"),
                    ...
                    R("앨범아트 크기", "그림과 같은 줄에서 맞춤 · 1:1 · 채우기 · 확대 축소",
                      "Album art size",
                      "The same row a picture gets - fit, 1:1, crop to fill, and the zoom stepper", "专辑封面大小", "与图片相同的一组模式：适应 · 1:1 · 填充裁剪 · 缩放步进"),
                    R("컨트롤 패널 왼쪽 두 칩", "백그라운드 재생 · 이어서 재생",
                      "The two chips at the left of the playback controls", "Background play · keep playing", "播放控制左侧的两个标签", "后台播放 · 继续播放"),
                    R("백그라운드 재생", "다른 폴더 이동해도 계속 (비활성화하면 정지 · 파일 해제)",
                      "Background play", "Carries on in other folders - switching it off stops it and frees the file", "后台播放", "切换到其他文件夹后仍继续；关闭后会停止播放并释放文件"),
                    R("이어서 재생 우클릭", "폴더 반복 · 한 곡 반복 · 셔플 반복",
                      "Right-click the keep-playing chip", "Repeat folder · repeat one · repeat shuffled", "右键“继续播放”标签", "重复文件夹 · 单曲循环 · 随机循环"),
                    R("음악 폴더 선택", "▶ 폴더 전체 재생 · 셔플 반복",
                      "Select a folder of music", "▶ play the folder, or repeat it shuffled", "选择音乐文件夹", "▶ 播放整个文件夹 · 随机循环"),
                    ...
                    R("폴더 우클릭 → 재생", "한 번이라도 펼쳐 본 폴더에 나타남",
                      "Right-click a folder → Play", "Appears for folders you have opened at least once", "右键文件夹 → 播放", "至少展开过一次的文件夹才会显示"),
                    R("재생 위치", "저장 안 됨 (이어보기는 영상만 지원)",
                      "Where you stopped", "Not remembered - only film carries on from where it was", "播放位置", "不会保存（只有视频支持继续播放）"),
                }),

                new Group(T("썸네일 바", "The thumbnail bar", "缩略图栏"), new[]
                {
                    R("장수 표시 옆 ▤ 버튼", "아래에 해당 폴더 이미지 목록 표시, 검색 중이면 검색 결과",
                      "The ▤ button beside the counter", "The folder's images as a row - or the results, while searching", "计数旁的 ▤ 按钮", "在下方显示该文件夹的图片列表；搜索时显示搜索结果"),
                    ...
                    R("옵션 → 멀티미디어 패널 → 썸네일 목록으로 보기", "여러 줄 목록(기본)과 한 줄 바 중 선택",
                      ...
                      "The multi-row list (the default) or the single-row bar", "选项 → 多媒体面板 → 缩略图网格", "在多行列表（默认）和单行缩略图栏之间选择"),
                    ...
                    R("바 위쪽 가장자리 드래그", "목록은 표시되는 줄 수, 바는 칸 크기 조정",
                      "Drag the top edge", "Rows on show in the list; cell size in the bar", "拖动顶部边缘", "列表调整显示的行数；缩略图栏调整单元格大小"),
                    R("목록에서 Ctrl+휠", "썸네일 크기 조정",
                      "Ctrl+wheel over the list", "Thumbnail size", "在列表上使用 Ctrl+滚轮", "调整缩略图大小"),
                    R("Ctrl+클릭 · Shift+클릭", "여러 개 선택 · 범위 선택 (Ctrl+Shift는 범위 추가, Ctrl+A는 전체)",
                      "Ctrl+click · Shift+click",
                      "Pick several · pick a range (Ctrl+Shift adds a range, Ctrl+A takes all)", "Ctrl+点击 · Shift+点击", "多选 · 选择范围（Ctrl+Shift 添加范围，Ctrl+A 全选）"),
                    R("선택한 칸에서 우클릭", "선택한 파일 전체를 복사 · 잘라내기 · 삭제",
                      "Right-click a picked cell", "Copy, cut or delete everything picked", "右键选中的单元格", "复制 · 剪切 · 删除所有选中的文件"),
                    R("칸을 밖으로 드래그", "다른 앱으로 드롭, 여러 개 선택했으면 함께 감",
                      "Drag a cell out", "Drop it into another app; a picked set travels together", "将单元格拖到外部", "拖放到其他应用；选中多个时会一起拖出"),
                    R("목록에 파일 끌어다 놓기", "보고 있는 폴더로 복사",
                      "Drop files onto the list", "They land in the folder on show", "将文件拖到列表上", "复制到当前查看的文件夹"),
                    R("옵션 → 멀티미디어 패널", "이미지 썸네일 캐싱 · 캐싱 파일 정리",
                      "Options → Multimedia panel", "Preload thumbnails · clean the cache up", "选项 → 多媒体面板", "预加载缩略图 · 清理缓存"),
                }),
            }),

        new Section(
            T("모양과 설정", "Appearance and settings", "外观和设置"),
            new[]
            {
                new Group(string.Empty, new[]
                {
                    R("옵션 → 색상 설정", "직접 선택 · 랜덤 지정", "Options → Color settings", "Pick them, or roll the dice", "选项 → 颜色设置", "手动选择 · 随机指定"),
                    ...
                    R("색상 설정 → 행의 고리", "색을 묶어 함께 변경, 함께 활성화한 줄끼리 같은 색",
                      "Color settings → the link on a row", "Ties rows together - the ones lit together share a color", "颜色设置 → 行链接", "将行绑定后一起更改；同时启用链接的行使用相同颜色"),
                    R("색상 설정 → 모노", "팔레트 전체를 회색톤으로 한 번에",
                      "Color settings → Mono", "The whole palette to grayscale in one press", "颜色设置 → 单色", "一键将整个调色板变为灰度"),
                    R("색상 설정 → 그림자", "트리와 북마크 패널의 위아래 끝을 옅게 덮음",
                      "Color settings → Shading", "Veils the top and bottom ends of the tree and the bookmark panel", "颜色设置 → 阴影", "淡淡覆盖树和书签面板的顶部与底部"),
                    R("옵션 → 기본 설정", "자동 시작 · 트레이 · 아이콘 · 자동 숨김", "Options → General", "Autostart · tray · icons · auto-hide", "选项 → 常规", "开机自启 · 托盘 · 图标 · 自动隐藏"),
                    ...
                    R("옵션 → 기본 설정 → 아이콘", "폴더 · 파일 · 드라이브를 따로따로 활성화/비활성화",
                      "Options → General → Icons", "Folders, files and drives switch on and off separately", "选项 → 常规 → 图标", "文件夹 · 文件 · 驱动器分别启用或禁用"),
                    R("옵션 → 기본 설정 → 드래그로 이동", "비활성화하면 드래그는 항상 복사, 이동은 Shift",
                      "Options → General → Drag Moves", "Off, a drag always copies and Shift is the way to move", "选项 → 常规 → 拖动移动", "关闭后，拖动始终执行复制；移动请按住 Shift"),
                    ...
                    R("옵션 → 기본 설정 → 경로 표시줄 위에 표시", "경로 줄을 트리 위로 이동",
                      "Options → General → Show path bar at top", "Moves the path line above the tree", "选项 → 常规 → 在顶部显示路径栏", "将路径栏移动到树的上方"),
                    ...
                    R("옵션 → 폴더에 표시할 항목 개수 → 전체 표시", "더 보기 없이 전부, 큰 폴더에서는 느려짐",
                      "Options → Items per folder → Show all", "No show more row at all - a big folder will feel it", "选项 → 每个文件夹显示的项目数 → 全部显示", "不显示“显示更多”行；大文件夹可能会变慢"),
                    R("옵션 → 네트워크 위치", "드라이브 문자 없는 공유 추가 (\\\\서버\\공유)",
                      "Options → Network locations", "Add a share with no drive letter (\\\\server\\share)", "选项 → 网络位置", "添加没有驱动器号的共享（\\server\\share）"),
                }),
            }),
    };
}
